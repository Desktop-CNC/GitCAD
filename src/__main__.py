...
def handle_fetch_repository(cwd: str):
    ...
    Handler.handle_repository_menu(
        ...
        err_msg=f"{Terminal.Text.RED}Failed to fetch the repository.{Terminal.Text.RESET} Press enter to continue."
    )
...
def handle_push_repository(cwd: str):
    ...
    Handler.handle_repository_menu(
        ...
        err_msg=f"{Terminal.Text.RED}Did not push changes.{Terminal.Text.RESET} It's possible there are no changes to push."
    )    
...
def main():
    ...
    main_menu.add_option("Fetch repository changes from GitHub", handle_fetch_repository, Handler.handle_github_current_working_directory)
    ...
    main_menu.add_option("Refresh dependencies", handle_fetch_dependencies, Handler.handle_github_current_working_directory)
